...
def run_client_transformer(rounds=3):
    client_name = "Transformer_Hyderabad"
    os.makedirs("logs", exist_ok=True)

    X, y, scaler = load_and_preprocess_data("private_datasets/Hyderabad_PM25.csv")
    public_X, _, _ = load_and_preprocess_data("public_dataset/public_data.csv")

    X = X.reshape((X.shape[0], X.shape[1]))
    public_X = public_X.reshape((public_X.shape[0], public_X.shape[1]))
