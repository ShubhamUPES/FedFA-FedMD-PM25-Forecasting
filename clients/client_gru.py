...
from utils.model_utils import build_gru_model
from utils.distill_utils import compute_logits, save_logits, load_consensus_logits
from sklearn.metrics import mean_squared_error, r2_score

def run_client_gru(rounds=3):
    client_name = "GRU_Bengaluru"
    os.makedirs("logs", exist_ok=True)

    X, y, scaler = load_and_preprocess_data("private_datasets/Bengaluru_PM25.csv")
    public_X, _, _ = load_and_preprocess_data("public_dataset/public_data.csv")

    X = X.reshape((X.shape[0], X.shape[1], 1))
    public_X = public_X.reshape((public_X.shape[0], public_X.shape[1], 1))
